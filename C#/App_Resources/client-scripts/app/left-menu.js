...
function BindMenuItems() {
    ...
    function () {
        ...
        //else if (
        //    currentVisibleMenuItem.parents('li').find("a[href='" + path + "']")[0] ||
        //    currentVisibleMenuItem.parents('li').find("a[href='" + path.replace("view", "edit") + "']")[0]
...
function LoadSelectedMenuItemOrNode() {
    ...
    var item = $("#menu").find("a[href='" + path.replace("edit", "view") + "']");
    if (item.length == 0)
        item = $("#menu").find("a[href='" + path.replace("view", "edit") + "']");

    ////hide existing visible menu item
    //$('#menu ul:visible').slideUp('normal');

    //hide existing visible menu item
    for (var i = 0; i < $('#menu ul:visible').length; i++) {
        var item2 = $($('#menu ul:visible')[i]).find("a[href='" + path.replace("edit", "view") + "']");
        if (item2.length == 0)
            item2 = $($('#menu ul:visible')[i]).find("a[href='" + path.replace("view", "edit") + "']");

        if (item2.length == 0) {// If is not the ul of the current page close
            $($('#menu ul:visible')[i]).slideUp('normal');
        }
    }
