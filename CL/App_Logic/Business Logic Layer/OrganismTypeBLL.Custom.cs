...
namespace Eisk.BusinessLogicLayer
{
    public partial class OrganismTypeBLL
    {
        ...
        public List<OrganismType> GetOrganismTypeByFilter(string organismType = default(string), string orderBy = default(string), int startRowIndex = 0, int maximumRows = 1)
        {
            ...
            HttpContext.Current.Items["organismTypes_Count"] = organismTypesVar.Count();

            var returnOrganismTypesVar = organismTypesVar.Skip(startRowIndex).Take(maximumRows).ToList();

            if (returnOrganismTypesVar.Count > 0)
            {
                var x = returnOrganismTypesVar[0].OrganismTypeName;
            }

            return returnOrganismTypesVar;
        }
        ...
        {
            var count = (int)HttpContext.Current.Items["organismTypes_Count"];
            HttpContext.Current.Items.Remove("organismTypes_Count");
            return count;
